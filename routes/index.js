...
router.get("/story", isLoggedIn, async function (req, res) {
  let user = await userModel.findOne({ username: req.session.passport.user }).populate("stories");
  if (!user || user.stories.length === 0) {
    return res.redirect("/feed");
  }
  let story = user.stories[user.stories.length - 1]; // latest story
  res.render("story", { footer: false, story, user });
});

router.get("/story/:id", isLoggedIn, async function (req, res) {
  let user = await userModel.findOne({ username: req.session.passport.user });
  let story = await storyModel.findOne({ _id: req.params.id }).populate("user");
  if (!story) return res.redirect("/feed");
  res.render("story", { footer: false, story, user });
});
